...
export class EditDeviceComponent implements OnInit {
  ...
  public addIO(device_index: any) {
    let index_key = 0;
    if (
      this.currentDevice.devices[device_index].hasOwnProperty("input_output")
    ) {
    ...
    if (this.validateForm()) {
      ...
      if (this.currentDevice.board_type === "raspberry_pi") {
        for (let i = 0; i < this.currentDevice.devices.length; i++) {
          for (
            let j = 0;
            j < this.currentDevice.devices[i].input_output.length;
            j++
          ) {
      ...
      this.deviceService.update(this.currentDevice).subscribe(
        data => {
  ...
  private validateForm(): Boolean {
    if (this.currentDevice.communication_protocols.length === 0) {
      this.alertService.error(
        "Specify at least one communication protocol",
        true
      );
      return false;
    }
    for (
      let i = 0;
      i < this.currentDevice.communication_protocols.length;
      i++
    ) {
      if (
        this.currentDevice.communication_protocols[i].bootstrap_servers
          .length === 0
      ) {
        this.alertService.error(
          "Specify at least one bootstrap server to send data",
          true
        );
        return false;
      } else {
        for (
          ...
          if (
            !this.validateIPAddress(
              this.currentDevice.communication_protocols[i].bootstrap_servers[j]
                .ip_address
            )
          ) {
    ...
    if (
      this.currentDevice.devices.length === 0 &&
      this.currentDevice.board_type === "raspberry_pi"
    ) {
  ...
  public addBootstrapServer(communication_protocol_index: any) {
    ...
    this.currentDevice.communication_protocols[communication_protocol_index][
      "bootstrap_servers"
    ].push({
      ip_address: "",
      port: 9092,
  ...
  public removeBootstrapServer(
    communication_protocol_index: any,
    bootstrap_server_index: any
  ) {
    this.currentDevice.communication_protocols[communication_protocol_index][
      "bootstrap_servers"
    ].splice(bootstrap_server_index, 1);
  }
  ...
  public removeIO(device_index: any, io_entry_index: any) {
    this.currentDevice.devices[device_index]["input_output"].splice(
      io_entry_index,
      1
    );
  }
